...
export const resendOtp = asyncHandler(async (req, res) => {
  const { email: rawEmail, reason } = req.body; // reason: 'registration' | 'forgot-password'
  const email = rawEmail.toLowerCase();
  const account = await findAccount(email);
  ...
  const otp = Math.floor(100000 + Math.random() * 900000).toString();
  account.otp = otp;
  account.otpExpire = Date.now() + 10 * 60 * 1000;
  await account.save();

  const isRegistration = reason === 'registration';
  const subject = isRegistration ? 'Verify Your Email - Dynamic Ticks' : 'Password Reset OTP - Dynamic Ticks';
  const title = isRegistration ? 'Account Verification' : 'Password Reset Protocol';
  const intro = isRegistration 
    ? 'Thank you for registering. Please use the following code to verify your email address:' 
    : 'A recovery request has been initiated for your account. Please use the following One-Time Password (OTP) to proceed:';
  ...
  console.log(`--- RESEND OTP (${reason.toUpperCase()}) ---`);
  console.log(`Email: ${account.email}`);
  console.log(`OTP: ${otp}`);
  console.log('---------------------------');
  ...
  try {
    ...
    res.status(200).json({
      message: 'OTP regenerated. (Email delivery failed - check server console for code)',
      emailError: err.message
    });
  }
});
...
export const googleAuth = asyncHandler(async (req, res) => {
  ...
  // Check if it's an ID Token (JWT with 3 parts) or an Access Token
