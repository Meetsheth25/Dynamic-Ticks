...
// @desc   Get all reviews (Manager/Staff)
// @route  GET /api/employees/reviews
...
// @desc   Delete a review (Manager/Staff)
// @route  DELETE /api/employees/reviews/:id
// @access Private/Staff,Manager
export const employeeDeleteReview = asyncHandler(async (req, res) => {
  ...
  // Update product's reviews stats
  const product = await Product.findById(productId);
  if (product) {
    const remainingReviews = await Review.find({ productId });
    product.reviews = remainingReviews.length;
...
// @desc   Get all users (Manager/Staff)
// @route  GET /api/employees/users
// @access Private/Staff,Manager
export const getUsersForEmployee = asyncHandler(async (req, res) => {
...
// @desc   Toggle user block (Manager/Staff)
// @route  PUT /api/employees/users/:id/block
// @access Private/Staff,Manager
export const toggleUserBlockForEmployee = asyncHandler(async (req, res) => {
  ...
  if (user.isAdmin) {
    res.status(403);
    throw new Error('Insufficient permissions to modify administrative accounts');
  }
