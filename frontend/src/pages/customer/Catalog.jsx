...
const Catalog = () => {
  ...
  useEffect(() => {
    ...
    // Gender Filter
    if (genderParam !== 'all') {
      result = result.filter(watch => watch.targetAudience?.toLowerCase() === genderParam.toLowerCase());
    }
  ...
  }, [categoryParam, genderParam, searchQuery, priceRange, sortBy, products]);
          ...
          {showFilters && (
            <aside className="w-full lg:w-72 shrink-0 animate-fade-in h-fit sticky top-32 z-20 bg-white">
              <div className="space-y-12">
                <div>
                  <h3 className="text-[11px] uppercase tracking-[0.3em] font-bold text-black mb-8 border-b-2 border-black/5 pb-4">
                    Price Parameters
                  </h3>
                  <div className="px-2">
                    <input 
                    ...
                    <div className="flex justify-between items-center mt-6">
                        <div>
                          <p className="text-[9px] uppercase tracking-widest text-gray-500 font-bold mb-1">Max Valuation</p>
                          <p className="text-sm font-black text-black tracking-widest bg-black/5 px-3 py-1.5 rounded-sm">Rs. {priceRange.toLocaleString()}</p>
                       </div>
                ...
                <div className="pt-8 border-t border-gray-50">
                   <h3 className="text-[10px] uppercase tracking-[0.3em] font-bold text-black mb-6">Demographic</h3>
                   <div className="space-y-4">
                      {[
                        { id: 'all', label: 'All Audences' },
                        { id: 'men', label: 'Men' },
                        ...
                        <label key={opt.id} className="flex items-center gap-4 cursor-pointer group">
                           <input 
                             ...
                             className="hidden" 
                             name="gender" 
                             checked={genderParam === opt.id}
                             onChange={() => {
                               const params = new URLSearchParams(searchParams);
                               if (opt.id === 'all') params.delete('gender');
                               else params.set('gender', opt.id);
                               setSearchParams(params);
                             }}
                           />
                           <div className={`w-3 h-3 border rounded-full transition-colors ${genderParam === opt.id ? 'border-black bg-black' : 'border-gray-200 group-hover:border-black'}`} />
                           <span className={`text-[10px] uppercase tracking-[0.15em] font-bold transition-colors ${genderParam === opt.id ? 'text-black' : 'text-gray-400 group-hover:text-black'}`}>{opt.label}</span>
                        </label>
                      ))}
                   </div>
                </div>
                      ...
                      {filteredWatches.map((watch, i) => (
                        ...
                        >
                          ...
                          <div className="text-center px-4">
                            ...
                            <p className="text-[10px] font-bold text-black tracking-[0.2em]">Rs. {watch.price?.toLocaleString()}</p>
                          </div>
                        </Link>
                      ))}
                    </div>
